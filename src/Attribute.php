...
final readonly class Attribute implements AttributeInterface
{
	...
	{
		return ReflectionManager::reflectClass($this->attribute)->newLazyGhost(
			fn(object $object) => $object->__construct(...$this->arguments),
		);
	}
	...
	public function getReflector(): Reflector
	{
		return match (true) {
			...
			&& null === $this->classConstant
			                               => ReflectionManager::reflectClass($this->class),
			null !== $this->method         => ReflectionManager::reflectMethod($this->class, $this->method),
			null !== $this->property       => ReflectionManager::reflectProperty($this->class, $this->property),
			null !== $this->classConstant  => ReflectionManager::reflectClassConstant($this->class, $this->classConstant),
			null !== $this->parameterIndex => ReflectionManager::reflectFunction($this->function)->getParameters()[$this->parameterIndex],
			null !== $this->function       => ReflectionManager::reflectFunction($this->function),
